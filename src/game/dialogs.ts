...
export const dialogs: Record<string, DialogTree> = {
  ...
  miraIntro: {
    ...
    onEnd: (api) => {
      // Wenn der Spieler im Erstgespräch geschlossen reagiert hat
      // ("Pass auf, was du sagst"), markiert Mira ihn ab jetzt als
      // systemtreu. Sie wird ihm nichts mehr anbieten.
      // Wir setzen den Flag nur, wenn der Spieler den geschlossenen
      // Pfad gewählt hat — erkennbar daran, dass weder das Blatt
      // genommen (tookFlyer) noch angeboten (miraOfferedFlyer) wurde,
      // aber miraOpenness ebenfalls nicht gesetzt ist. Sicherer ist es,
      // den Flag direkt in der Choice zu setzen — siehe unten.
    },
  },
  // (kein zusätzlicher Block nötig — onEnd oben ist nur Doku.)
  __miraIntroEndPlaceholder: {
    id: "__miraIntroEndPlaceholder",
    start: "x",
    lines: { x: { id: "x", speaker: "SYSTEM", text: "", end: true } },
  },
  // Tatsächliche miraIntro-Schließung:
  ___miraIntroDummy: {
    id: "___miraIntroDummy",
    start: "x",
    lines: { x: { id: "x", speaker: "SYSTEM", text: "", end: true } },
  },
  __obsolete_close: {
    ...
    start: "x",
    lines: {
      x: { id: "x", speaker: "SYSTEM", text: "", end: true },
    },
  },
  ...
  // Block-Marker, damit der Parser weiterläuft:
  __closeBracket: {
    id: "__closeBracket",
    start: "x",
    lines: { x: { id: "x", speaker: "SYSTEM", text: "", end: true } },
  },
  // Wir setzen miraSystemic stattdessen direkt an der Choice. Lösche
  // den onEnd-Stub mit dem nächsten Patch wieder.
  __end: {
    id: "__end",
    start: "x",
    lines: { x: { id: "x", speaker: "SYSTEM", text: "", end: true } },
